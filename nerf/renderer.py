...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, perturb=False, **kwargs):
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, perturb=False, force_all_rays=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096, bg_color=None, perturb=False, **kwargs):
        ...
        if not staged or self.cuda_ray:
            return _run(rays_o, rays_d, bg_color=bg_color, perturb=perturb, **kwargs)
